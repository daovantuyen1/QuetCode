    ...
    public class AuthenToken
    {
        ...
        public static string GetToken(string userName,string mail,string phone)
        {
            try
            {
                ...
                permClaims.Add(new Claim("aa", Encode.Encrypt(!string.IsNullOrWhiteSpace(userName)? userName:"aa", Constant.KEY_ENCODE)));   //userName
                permClaims.Add(new Claim("bb", Encode.Encrypt(!string.IsNullOrWhiteSpace(mail)? mail:"bb", Constant.KEY_ENCODE)));   //mail
                permClaims.Add(new Claim("cc", Encode.Encrypt(!string.IsNullOrWhiteSpace(phone)? phone:"cc", Constant.KEY_ENCODE)));   //phone
                permClaims.Add(new Claim("dd", Encode.Encrypt(Util.GetIp(), Constant.KEY_ENCODE))); // IP
        ...
        public static bool CheckTokenValid(string token)
        {
            try
            {
                ...
                string IP = Encode.Decrypt(tokenJWT.Payload.ToList()[4].Value.ToString(), Constant.KEY_ENCODE);
                if (IP == Util.GetIp())
                {
                    return true;
                }
